...
-- there are two tables YY = Policy Year roll up, YM for Policy Year Month detail

if exists(select OBJECT_ID from pdb_Allsavers_Research.sys.objects where name = 'MemberClaimDetailYM')
   drop table pdb_Allsavers_Research.dbo.MemberClaimDetailYM;

CREATE TABLE dbo.MemberClaimDetailYM(
...
insert into pdb_Allsavers_Research.dbo.MemberClaimDetailYM
select mb.Member_DIMID, mb.PolicyYear
	 , Year_Mo            = cast(mb.YearMo as int)
...
create clustered index ucix_Member_DIMID on pdb_Allsavers_Research.dbo.MemberClaimDetailYM (Member_DIMID);
...
select distinct --top 100         -- intermediate catalyst table 
       yy.Member_DIMID, yy.PolicyYear, mb.YearMo, EffDtSysID = mb.MinDtSysId, EndDtSysID = mb.MaxDtSysID
     , xm.SystemID
  into #CLMMembers
  from MemberClaimDetailYY      yy
  join #mbrBasis mb on yy.Member_DIMID = mb.Member_DIMID
...
update yy
  ...
  from MemberClaimDetailYM yy
  join (
SELECT cg.Member_DIMID, cg.PolicyYear, cg.YearMo
 ...
 Group by cg.Member_DIMID, cg.PolicyYear, cg.YearMo
       ) clm on yy.Member_DIMID = clm.Member_DIMID 
	        and yy.PolicyYear = clm.PolicyYear and yy.Year_Mo = clm.YearMo
;

--TP: 
select count(member_dimid) 
  from MemberClaimDetailYM 
 where PaidAmt = 0.0 and AllwAmt = 0.0
 order by 1,2,3
