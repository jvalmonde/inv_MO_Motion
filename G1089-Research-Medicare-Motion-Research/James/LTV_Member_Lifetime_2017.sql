...
join pdb_WalkandWin..LTV_Member_Enrollment					as c	on a.SavvyHICN = c.SavvyHICN
...
join pdb_WalkandWin..LTV_Member_Enrollment					as d	on c.SavvyHICN = d.SavvyHICN
...
from pdb_WalkandWin..LTV_Member_Enrollment	as a 
...
-------------------------------------------------------------
-- drop table pdb_WalkandWin..LTV_Member_Lifetime
select a.*, 
...
into pdb_WalkandWin..LTV_Member_Lifetime
from(
	select a.*, b.New_Member_Status
	from pdb_WalkandWin..LTV_Member_Lifetime_ID	as a
	join #previous_enrollment	as b	on a.SavvyHICN = b.SavvyHICN and a.Lifetime_ID = b.Lifetime_ID
	) as a
	-- 50901

create unique index ix_SH_LI on pdb_WalkandWin..LTV_Member_Lifetime(SavvyHICN, Lifetime_ID)
create unique index ix_SH_EY on pdb_WalkandWin..LTV_Member_Lifetime(SavvyHICN, Enroll_Year_Mo)
create unique index ix_SH_DY on pdb_WalkandWin..LTV_Member_Lifetime(SavvyHICN, Disenroll_Year_Mo)

select distinct savvyhicn from pdb_WalkandWin..LTV_Member_Lifetime


select * from pdb_WalkandWin..LTV_Member_Lifetime as a
...
SELECT  a.[SavvyHICN]
      ...
      ,[New_Member_Status]
      ,[Lifetime_Type]
into  [pdb_GP1026_WalkandWin_Research].[final].[LTV_Member_Lifetime2]
  FROM [pdb_GP1026_WalkandWin_Research].[final].[LTV_Member_Lifetime] as a
