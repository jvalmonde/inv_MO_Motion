...
\title{All Savers Motion - Phase 2 Campaign 2: GP0284}
\maketitle
\clearpage
...
all_dat=all_dat[DateBenefitsEnd>'2016-06-30' & RegisteredBeforetouch2==0]
all_dat[is.na(Touch2Treatment),Touch2Treatment:='Control']
setnames(all_dat,'Touch2Treatment','Treat2')
all_dat=all_dat[AccountVerifiedDateTime>"2016-06-28" | is.na(AccountVerifiedDateTime) & !is.na(Treat2)]
all_dat[,Treat2:=ifelse(Treat2==215,'ActNow_Letter_NewPS',ifelse(Treat2==216,'Benefit_Letter_OldPS',ifelse(Treat2==217,'ActNow_Comic_OldPS',ifelse(Treat2==218,'Benefit_Comic_NewPS','Control'))))]
  
all_dat[,c('TreatmentMethod2','TreatmentFormat2','TreatmentPS'):=list(ifelse(Treat2%in%c('ActNow_Letter_NewPS','ActNow_Comic_OldPS'),'ActNow',ifelse(Treat2%in%c('Benefit_Letter_OldPS','Benefit_Comic_NewPS'),'Benefit','Control')),
                                                                      ifelse(Treat2%in%c('ActNow_Letter_NewPS','Benefit_Letter_OldPS'),'Letter',ifelse(Treat2%in%c('ActNow_Comic_OldPS','Benefit_Comic_NewPS'),'Comic','Control')),
                                                                      ifelse(Treat2%in%c('ActNow_Letter_NewPS','Benefit_Comic_NewPS'),'New',ifelse(Treat2%in%c('ActNow_Comic_OldPS','Benefit_Letter_OldPS'),'Old','Control')))]
all_dat[,Treat2:=factor(Treat2,levels=c('Control','ActNow_Comic_OldPS','ActNow_Letter_NewPS','Benefit_Letter_OldPS','Benefit_Comic_NewPS'))]
...
tmp=data.table(Dates=seq(as.Date("2016-06-28"),as.Date(Sys.time(),tz=""),by="1 day"),Value=0)
@
...
by_vars1=c('AccountVerifiedDateTime','Treat2')
by_vars2=c('Treat2')
ttt1=all_dat[,.N,by=by_vars1][,list(c(AccountVerifiedDateTime,tmp$Dates[!tmp$Dates%in%AccountVerifiedDateTime]),
                               ...
                               c(cumsum(V2[!is.na(V1)]),0),sum(V2)),by=by_vars2][!is.na(V1),list(V1,V2,V3,V2/V3),by=c(by_vars2)][order(Treat2)]
setnames(ttt1,names(ttt1),c(by_vars2,'AccountVerifiedDateTime','Registered','N','Proportion'))

ggplot(ttt1,aes(x=AccountVerifiedDateTime,y=Proportion,color=Treat2))+
  geom_point()+
  geom_line()+
...
pdf('~/GP0284-Research-MotionRegistration/DanH/Phase2Touch2_WriteUp.pdf',width=11,height=8.5)
ggplot(ttt1,aes(x=AccountVerifiedDateTime,y=Proportion*100,color=Treat2))+
  geom_line(size=1)+
  geom_line()+
...
tmp2=all_dat[,list(sum(IsRegistered),.N),by=c('LookupRuleGroupid','Treat2')]

glm1=glm(V1~Treat2+offset(log(N)),data=tmp2,family='quasipoisson')
...
by_vars1=c('AccountVerifiedDateTime','TreatmentMethod2')
by_vars2=c('TreatmentMethod2')
ttt1=all_dat[,.N,by=by_vars1][,list(c(AccountVerifiedDateTime,tmp$Dates[!tmp$Dates%in%AccountVerifiedDateTime]),
                                    c(N,tmp$Value[!tmp$Dates%in%AccountVerifiedDateTime])),by=by_vars2][order(V1)][,list(V1,
                                    c(cumsum(V2[!is.na(V1)]),0),sum(V2)),by=by_vars2][!is.na(V1),list(V1,V2,V3,V2/V3),by=c(by_vars2)][order(TreatmentMethod2)]
setnames(ttt1,names(ttt1),c(by_vars2,'AccountVerifiedDateTime','Registered','N','Proportion'))

ggplot(ttt1,aes(x=AccountVerifiedDateTime,y=Proportion,color=TreatmentMethod2))+
  geom_point()+
  geom_line()+
...
tmp2=all_dat[,list(sum(IsRegistered),.N),by=c('LookupRuleGroupid','TreatmentMethod2')]
glm1=glm(V1~TreatmentMethod2+offset(log(N)),data=tmp2,family='quasipoisson')
p1=coef(summary(glm1))[2,4]
p2=coef(summary(glm1))[3,4]
...
by_vars1=c('AccountVerifiedDateTime','TreatmentFormat2')
by_vars2=c('TreatmentFormat2')
ttt1=all_dat[,.N,by=by_vars1][,list(c(AccountVerifiedDateTime,tmp$Dates[!tmp$Dates%in%AccountVerifiedDateTime]),
                                    c(N,tmp$Value[!tmp$Dates%in%AccountVerifiedDateTime])),by=by_vars2][order(V1)][,list(V1,
                                    c(cumsum(V2[!is.na(V1)]),0),sum(V2)),by=by_vars2][!is.na(V1),list(V1,V2,V3,V2/V3),by=c(by_vars2)][order(TreatmentFormat2)]
setnames(ttt1,names(ttt1),c(by_vars2,'AccountVerifiedDateTime','Registered','N','Proportion'))

ggplot(ttt1,aes(x=AccountVerifiedDateTime,y=Proportion,color=TreatmentFormat2))+
  geom_point()+
  geom_line()+
...
tmp2=all_dat[,list(sum(IsRegistered),.N),by=c('LookupRuleGroupid','TreatmentFormat2')]
glm1=glm(V1~TreatmentFormat2+offset(log(N)),data=tmp2,family='quasipoisson')
p1=coef(summary(glm1))[2,4]
p2=coef(summary(glm1))[3,4]
...
\clearpage
The following figure shows how the proportion of registered members for each of the treatment PS statements has changed since the mailing. The table gives the current value of each treatment PS groups.\\ 
\\
<<TreatmentPS,results=hide,echo=FALSE,fig=TRUE>>=
by_vars1=c('AccountVerifiedDateTime','TreatmentPS')
by_vars2=c('TreatmentPS')
ttt1=all_dat[,.N,by=by_vars1][,list(c(AccountVerifiedDateTime,tmp$Dates[!tmp$Dates%in%AccountVerifiedDateTime]),
                                    c(N,tmp$Value[!tmp$Dates%in%AccountVerifiedDateTime])),by=by_vars2][order(V1)][,list(V1,
                                    c(cumsum(V2[!is.na(V1)]),0),sum(V2)),by=by_vars2][!is.na(V1),list(V1,V2,V3,V2/V3),by=c(by_vars2)][order(TreatmentPS)]
setnames(ttt1,names(ttt1),c(by_vars2,'AccountVerifiedDateTime','Registered','N','Proportion'))

ggplot(ttt1,aes(x=AccountVerifiedDateTime,y=Proportion,color=TreatmentPS))+
  geom_point()+
  geom_line()+
  ylab('Proportion Registered')+xlab('Date')+ggtitle("Proportion by Treatment PS")+
  scale_color_discrete(name="")+
  theme(legend.position='bottom')
...
tmp2=all_dat[,list(sum(IsRegistered),.N),by=c('LookupRuleGroupid','TreatmentPS')]
glm1=glm(V1~TreatmentPS+offset(log(N)),data=tmp2,family='quasipoisson')
p1=coef(summary(glm1))[2,4]
p2=coef(summary(glm1))[3,4]
...
<<TreatmentPSTable,results=tex, echo=FALSE>>=
print(xtab1, caption.placement = "top",include.rownames=getOption("xtable.include.rownames", FALSE))
@
...
by_vars1=c('AccountVerifiedDateTime','Treat2','BinAge','OthersReg','GenderCode')
by_vars2=c('Treat2','BinAge','OthersReg','GenderCode')
ttt1=all_dat[,.N,by=by_vars1][,list(c(AccountVerifiedDateTime,tmp$Dates[!tmp$Dates%in%AccountVerifiedDateTime]),
                                    c(N,tmp$Value[!tmp$Dates%in%AccountVerifiedDateTime])),by=by_vars2][order(V1)][,list(V1,
                                    c(cumsum(V2[!is.na(V1)]),0),sum(V2)),by=by_vars2][!is.na(V1),list(V1,V2,V3,V2/V3),by=c(by_vars2)][order(Treat2)]
...
col9=all_dat[,c(as.character(round(sum(IsRegistered)/.N,3)),paste(sum(IsRegistered),'/',.N,sep="")),keyby=c('Treat2')]$V1
...
DOEtable=all_dat[,c(.N,.N),keyby=c('Treat2','TreatmentMethod2','TreatmentFormat2')]
DOEtable[,V1:=NULL]
...
DOEtable[,Treat2:=NULL]
DOEtable[,c('TreatmentMethod2','TreatmentFormat2'):=list(as.character(TreatmentMethod2),as.character(TreatmentFormat2))]
DOEtable[(1:nrow(DOEtable)%%2)==0,c('TreatmentMethod2','TreatmentFormat2'):=list(NA,NA)]
...
# View(all_dat[,list(sum(IsRegistered),.N,sum(IsRegistered)/.N),by=c('LookupRuleGroupid','Treat2')][order(-N)])
row.ind=nrow(all_dat[OthersReg=='Yes',.N,by=LookupRuleGroupid])
tmp1=all_dat[OthersReg=='Yes',list(sum(IsRegistered),.N,sum(IsRegistered)/.N),by=c('LookupRuleGroupid','GroupEffectiveDate','Treat2')][order(-V1)][1:7]
tmp2=all_dat[OthersReg=='Yes',list(sum(IsRegistered),.N,sum(IsRegistered)/.N),by=c('LookupRuleGroupid','GroupEffectiveDate','Treat2')][order(-V1)][8:row.ind,list(sum(V1),sum(N),sum(V1)/sum(N))]
tmp1=tmp1[,list(c(LookupRuleGroupid,'Combined Top 7',paste('All Others (',row.ind-7,')',sep="")),c(as.character(GroupEffectiveDate),NA,NA),c(as.character(Treat2),'Combined Top 7',paste('All Others (',row.ind-7,')',sep="")),c(V1,sum(V1),tmp2$V1),c(N,sum(N),tmp2$V2),c(V3,sum(V1)/sum(N),tmp2$V3))]
...
tmp1=all_dat[OthersReg=='No',list(sum(IsRegistered),.N,sum(IsRegistered)/.N),by=c('LookupRuleGroupid','GroupEffectiveDate','Treat2')][order(-V1)][1:7]
tmp2=all_dat[OthersReg=='No',list(sum(IsRegistered),.N,sum(IsRegistered)/.N),by=c('LookupRuleGroupid','GroupEffectiveDate','Treat2')][order(-V1)][8:row.ind,list(sum(V1),sum(N),sum(V1)/sum(N))]
tmp1=tmp1[,list(c(LookupRuleGroupid,'Combined Top 7',paste('All Others (',row.ind-7,')',sep="")),c(as.character(GroupEffectiveDate),NA,NA),c(as.character(Treat2),'Combined Top 7',paste('All Others (',row.ind-7,')',sep="")),c(V1,sum(V1),tmp2$V1),c(N,sum(N),tmp2$V2),c(V3,sum(V1)/sum(N),tmp2$V3))]
setnames(tmp1,names(tmp1),c('CompanyID','GroupDate','Treatment','Registered','Total','Proportion'))
...
all_dat[,StartTime:=as.numeric(round(difftime('2016-06-28',EffectiveDate,units='days'),0))]
all_dat[,EndTime:=StartTime+as.numeric(round(difftime(BenefitEndDate,'2016-06-28',units='days'),0))]
...
fit.Registered=summary(survfit(Surv(StartTime,Time2, registered2==1)~TreatmentMethod2, data= all_dat),time=0:md)
# summary(fit.Registered,time=0:60)
# survdiff(Surv(TimetoRegister, event=IsRegistered==1)~TreatmentMethod2, data= all_dat) 
# summary(m1 <- coxph(Surv(TimetoRegister, event=IsRegistered==1)~TreatmentMethod2, data= all_dat)) 
...
tmp=all_dat[,list(sum(registered2==1),.N),by=c('Time2','Treat2')][order(Time2)]
tmp2=all_dat[,.N,by=Treat2]
tmp2=tmp2[,list(rep(Treat2,each=length(min(tmp$Time2):max(tmp$Time2))),min(tmp$Time2):max(tmp$Time2),0)]
