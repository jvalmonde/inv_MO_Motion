...
into #BaseMembership
FROM Dermsl_prod.dbo.MemberSignupData 
where activeflag = 1 and programstartdate between '20160701' and '20160801' and AccountVerifiedFlag = 0 and isnull(CancelledDatetime,getdate()) >= getdate() and lookupclientid = 50 
...
,GroupEffectiveDate = a.EffectiveDate
...
--Select * FROM #AdminContactData where PolicyId = '5400-2050'
...
Select b.*, a.[Executive Contact], ExecandAdminDifferent = Case when a.[Executive Contact]  <> a.[Email First ] + ' ' + [Email Last] then 1 else 0 end 
Into pdb_AllSaversRegistration..DirectMailList_Phase3_Campaign1
FROM    #MembershipandContactData b  							
		Left join dbo.ExecutiveContacts_NewGroups	a  	On  a.Assoc + '-' + a.Client =  b.PolicyID 
...
--Select * FROM Dermsl_prod.dbo.membersignupdata where rowCreatedDateTime >= '20160603'
 ...
 Select * FROM pdb_AllSaversRegistration..DirectMailList_Phase3_Campaign1 Where PolicyId = '5400-4597'

 
Select * FROM Dim_SESAllsaversPolicyYear where offercode = '5400-4597'