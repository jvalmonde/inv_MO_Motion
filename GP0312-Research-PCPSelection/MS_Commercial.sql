/*** 
PCP Selection (Commercial population) Master Script

Input databases:	MiniHPDM, pdb_PCPSelection

Date Created: 12 January 2017
***/
...
--(24533 row(s) affected); 7.17 minutes
...
--pull for the claims base table
...
select a.Indv_Sys_Id, Provider_MPIN = a.MPIN, Provider_TIN = a.TIN, EFNCY_FLAG = max(a.EFNCY_FLAG), QLTY_FLAG = max(a.QLTY_FLAG)
	, PCP_Flag	= max(isnull(a.PCP_Flag,0))	--added 1/11/2017; consolidate data per member, MPIN, TIN combo level as per Matt
	...
	, EndDate	= max(Full_Dt)
into pdb_PCPSelection..Com_Prov_Claims_10				
from	(
			select c.*
...
--(2,245,576 row(s) affected); 46.19 minutes
...
select Indv_Sys_Id
	, Splitter = max(Splitter)																--member splits time with multiple PCPs
	, Switcher = case when max(Splitter) = 0 and max(x.OID) <> 1	then 1	else 0	end		--member switches to different PCPs
	, Sticker = case when max(x.OID) = 1	then 1	else 0 end								--member has 1 PCP only
into #class		
from	(	
...
--member classification consolidated
...
--build member class table; add weight
...
select a.Indv_Sys_ID
	...
	, Splitter	
	, Sticker
	, Weight = (1 - b.Percnt)	--% of visits to PCPs	
	, Total_IP_Spend	
	, Total_OP_Spend	
	, Total_DR_Spend	
	, Total_ER_Spend
