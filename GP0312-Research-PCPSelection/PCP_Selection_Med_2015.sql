...
Input databases:	MiniOV, pdb_PCPSelection
...
select b.SavvyHICN, b.SavvyID, Admt_DtSys = c.Dt_Sys_Id, Discharge_DtSys = (c.Dt_Sys_Id + c.Day_Cnt)
into #ip_conf
...
where d.YEAR_NBR = 2015
	...
	and c.Admit_Cnt = 1
group by b.SavvyHICN, b.SavvyID, c.Dt_Sys_Id, c.Day_Cnt
--(38931 row(s) affected); 
create clustered index cIx_SavvyID on #ip_conf (SavvyHICN, SavvyID);
...
if object_id('pdb_PCPSelection..Prov_Claims_10_2015') is not null
drop table pdb_PCPSelection..Prov_Claims_10_2015
go
...
select a.SavvyHICN, Provider_MPIN = a.MPIN, Provider_TIN = TIN, EFNCY_FLAG, QLTY_FLAG
	...
	, EndDate	= max(Full_Dt)
into pdb_PCPSelection..Prov_Claims_10_2015
from	(		
...
--(1387448 row(s) affected); 21.02 minutes
create clustered index cIx_HICN_MPIN on pdb_PCPSelection..Prov_Claims_10_2015 (SavvyHICN, Provider_MPIN);
...
from	(	
			...
			from	(	
						...
						from pdb_PCPSelection..Prov_Claims_10_2015
						where PCP_Flag = 1
					)	a
			left join (	
						select SavvyHICN, Provider_MPIN, StartDate, EndDate, OID = row_number() over (partition by SavvyHICN order by StartDate)
...
where b.ESRDFlag = 'Y'
	and year(b.PaymentAdjustmentStartDate) = 2015
group by a.SavvyHICN, ESRDFlag
--(885 row(s) affected); 4.06 minutes
...
from(
	...
	join #ip_conf				as b	on a.SavvyId = b.SavvyId
										   and a.Dt_Sys_Id between b.Admt_DtSys and b.Discharge_DtSys
...
where d.YEAR_NBR = 2015
--(2444844 row(s) affected); 5.14 minutes
...
Into	#HCC
From	(Select	* 
		From	pdb_PCPSelection..RA_ModelTerms_2015
		)										as	a
left join	[RA_Medicare_2014_v2 ]..ModelTerm			as	b	on	a.Term = b.Term
where b.ModelID = 6
--(210373 row(s) affected)


------------------------------------------------------------------------------------------------------------------
--Dynamic Pivot:
if OBJECT_ID ('pdb_PCPSelection..HCC_2015') is not null begin drop table pdb_PCPSelection..HCC_2015	End;
...
into pdb_PCPSelection..HCC_2015
from
...
left join pdb_PCPSelection..RA_RAF_2015	d	on	a.SavvyHICN = d.UniqueMemberID
--(155766 row(s) affected); 
create unique clustered index ucIx_HICN on #memberclass (SavvyHICN);

--add weight
if object_id('pdb_PCPSelection..MbrClass_10_2015') is not null
drop table pdb_PCPSelection..MbrClass_10_2015
go
...
select a.SavvyHICN
    ...
    , DISABLED_HCC46					= isnull(d.DISABLED_HCC46				, 0)
...
into pdb_PCPSelection..MbrClass_10_2015
from #memberclass	a
inner join	(
				...
				from	(
							...
							from #memberclass	a
							left join pdb_PCPSelection..Prov_Claims_10_2015	b	on	a.SavvyHICN = b.SavvyHICN
							group by a.SavvyHICN
						) a
				left join pdb_PCPSelection..Prov_Claims_10_2015	b	on	a.SavvyHICN = b.SavvyHICN
			)	b	on	a.SavvyHICN = b.SavvyHICN
...
left join pdb_PCPSelection..HCC_2015	d	on	a.SavvyHICN = d.UniqueMemberID
where b.RN = 1
--(155766 row(s) affected)
create unique clustered index ucIx_HICN on pdb_PCPSelection..MbrClass_10_2015	(SavvyHICN);


if object_id('pdb_PCPSelection..MbrProv_10_2015') is not null
drop table pdb_PCPSelection..MbrProv_10_2015
go
...
into pdb_PCPSelection..MbrProv_10_2015
from pdb_PCPSelection..MbrClass_10_2015			a
left join pdb_PCPSelection..Prov_Claims_10_2015	b	on	a.SavvyHICN = b.SavvyHICN
--(1408704 row(s) affected)
...
from	(	
			...
			from	(	
						...
						from pdb_PCPSelection..MbrProv_10_2015
						where Switcher = 1 
							and PCP_Flag = 1
					) a
			left join	(
...
--(23151 row(s) affected)
create unique clustered index ucIx_HICN on #dateswitch (SavvyHICN);

alter table pdb_PCPSelection..MbrClass_10_2015
	add DateSwitch	date
		, SwitchCnt	smallint
		, DaysToFrom_IPConf	int
go

update pdb_PCPSelection..MbrClass_10_2015
...
--(258081 row(s) affected); 101 lost MPINs
create unique clustered index ucIx_MPIN on #ntwrk_sts (Provider_MPIN);

alter table pdb_PCPSelection..MbrProv_10_2015
	add Network_Flag	smallint
go

update pdb_PCPSelection..MbrProv_10_2015
set Network_Flag = isnull(b.Network_Flag, 0)
from pdb_PCPSelection..MbrProv_10_2015	a
...
select * from pdb_PCPSelection..MbrClass_10_2015