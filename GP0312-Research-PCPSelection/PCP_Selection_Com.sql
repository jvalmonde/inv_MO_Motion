/*** 
PCP Selection (Commercial population)
...
select top 10 percent b.*
into #member
from MiniHPDM..Dim_CustSegSysId	a
inner join MiniHPDM..Dim_Member	b	on	a.Cust_Seg_Sys_Id = b.Cust_Seg_Sys_Id
...
where IP_Spend = 0
	and IP_SrvcCnt > 0
--NOTE:  these 12 member-provider combo have reversed claims; allowed amount were offset

select *
from #ip_conf
where Indv_Sys_Id in (32133786, 854332796, 908304601)

select Indv_sys_ID, sum(allw_amt)
from #temp
where Derived_Srvc_Type_cd = 'IP'
group by Indv_sys_ID

select *
from #temp
where Indv_sys_ID = 908304601
	and MPIN = 372530
*/
