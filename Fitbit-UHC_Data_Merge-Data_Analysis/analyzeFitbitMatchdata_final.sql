/*
prepare data to analyze United members with Fitbits based on Data Services' matching. 

Before we look at the characteristics of members with Fitbits I would like to better understand which members to potentially include. 
Initially we’re interested in broad comparisons, and if we find something interesting we can further drill down. I’m looking at a single 
...
I am not currently including RAF/HCCs. We can drill in further in the future, but the Charlson scores I’m using are much easier 
(at least computationally) to compute. Either serves as an imperfect proxy for health status, so for now we’ll save RAF/HCCs for 
future exploration. In addition, the sample sizes we’re working with are large, so I can afford to paint with a broad brush and 
use simple rules. For example, I’m using
	Adults = born before 1999
	all members continuously enrolled in the same plan during 2017
	Charlson scores and indicators built requiring only a single diagnosis during 2017, with no filters based on visit type, 
	place of service, provider or the like.

As an example of how to interpret these scores, ‘Diabetes’ for a member wouldn’t indicate that the member meets clinical criteria 
for diabetes, just that she had at least one diagnosis in 2017 related to diabetes. If the ‘Diabetes’ rate in a group of members is 6.8%, 
that just means it’s likely that more people in the group have diabetes than in a different group with a 5.9% rate.

You'll need access to
	pdb_UHCEmails	(our working database)
	MiniHPDM		(we asked United for emails only for commercial members)
	MiniHPDM_PHI	(not strictly necessary, but we restricted on birth date to create a simple rule for selecting adult members)
	Fitbit_Match	(DS did the matching here)
	Charlson		(to get diagnoses by condition)

End result will be Member_Continuous_2017_HPDM_Summary, which will have 1 row per member with details about the member,
their health plan/email/fitbit status, utilization, and disease (or at least condition) status. 

All data exploration for my analysis (if I have indeed included everything I need) can then use single-table queries from this table.
*/
...
--we'll add member information later, but for performance we'll minimize tables referenced here
--this will give current customer segment and Insurance (company/financing) based on the end 2017, 
--but that'll be accurate enough for our purposes and is pretty easy to get at.
	...
	from
				MiniHPDM.dbo.Summary_Indv_Demographic				a
...
--get a list of members for whom we have an email address.
--we matched to Fitbit on email address, so these are the only members who we'd potentially match
...
--get data from various dimension tables
--restrict to adults (born before 1999)
--one row per member
	...
	where 1=1
		...
		and m.Gdr_Cd						<>	'U'					--just a few, so we'll filter these out without affecting our conclusions
		and mp.Bth_dt						<=	'1998-12-31'
	go
	create clustered index cixIndv_Sys_Id on dbo.Member_Continuous_2017_HPDM_Include(Indv_Sys_Id)
	go

--get Charlson scores and diagnoses for our members. Again, I'm not interested in clinically accurate diagnoses, just in broad 
--differences between two (fairly large groups). So I'll just consider whether a member ever had a diagnosis related to the 
--conditions

	--for efficiency, grab distinct member conditions, then compute charlsons from these.
	--separate into three subqueries to maximize possible advantage of indexes on Fact_Claims.Diag_?_Cd_Sys_Id.
	...
	--Build Charlson scores and indicators. 
	--I'll use only the Condition_1 variables, which require only a single diagnosis during 2017, with no filters based on 
	--visit type, place of service, provider or the like. I also compute the *_2, because I stole this code and there's little cost 
	--to adding to the calculation.
...
--calculate utilization for the members. Again (again!), I'm only looking for broad differences.
--we'll use DerivedSrvcTyp to attribute all costs during IP admissions to IP

	--get a list of days during which members are admitted to Inpatient Hospitals 
		...
		--pull claims for the members. All claims during days they're in the hospital will be classified as Ip.
		...
		select 
			...
			fc.Vst_Cnt,
			DerivedSrvcTyp						=	case 
														when ip.Indv_Sys_Id is not null	then 'IP'		--date is during an IP hospital admission
		...
		--summarize utilization by member
...
--for efficiency of calculation pull it all together 
--I'll include redundant indicators and descriptions to give flexibility and help keep calculations legible. 
--As an example, we include both Gdr_Cd (so we can segment by gender and have F called F) and 
--isFemale (so I can get use avg(isFemale) instead of avg(case when Gdr_Cd = 'F' then 1 else 0 end))
